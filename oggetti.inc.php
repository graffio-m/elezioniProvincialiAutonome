    ...
    public function setVotiListeCandidatoTrento($dataVotiListeAr) {
        ...
        if (count($dataVotiListeInCorsoAr) > 0 ) {
         ...
         foreach ($dataVotiListeInCorsoAr as $singolaLista) {
            $voti_validi_comune = $this->jsonObject->cand[$this->numeroCandidato]->voti_validi_comune;
            if ($this->jsonObject->cand[$this->numeroCandidato]->id_Presidente == $singolaLista['Presidente Id']) {
                ...
                if ($this->jsonObject->int->cod_ISTAT == $singolaLista['Istat Comune']) {
                    ...
                    $this->jsonObject->cand[$this->numeroCandidato]->liste[$this->numeroLista]->img_lis_c = $singolaLista['img_lis_c']; 
 ...
 */                 
                    if ($singolaLista['Voti'] > 0 && $voti_validi_comune > 0) {
                        $percVotiLista = round((($singolaLista['Voti']/$voti_validi_comune)*100),2);
                    }
 ...
 */        });
    ...
    public function setCandidatoBolzano($candidatoAr) {
        ...
        $votiValidi = $candidatoAr['voti_validi'];
...
//        $votiValidi = $this->jsonObject->int->vot_t - ($candidatoAr['MUNI_BLANK'] + $candidatoAr['MUNI_NULLS']);
    ...
    public function setCandidatoTrento($candidatoAr) {
        ...
        $this->jsonObject->cand[$this->numeroCandidato]->voti_validi_comune = $candidatoAr['voti_validi_comune']; 
        $this->jsonObject->cand[$this->numeroCandidato]->voti_validi_provincia = $candidatoAr['voti_validi_provincia']; 
 ...
 */        switch ($this->jsonObject->int->cod_prov) {
    ...
    public function setVotiListeCandidatoProvinciaTrento($dataVotiListeHA, $comuneInCorso) {
        $voti_validi_provincia = $this->jsonObject->cand[$this->numeroCandidatoProvincia]->votiValidi;
        $idPresidente = $this->jsonObject->cand[$this->numeroCandidatoProvincia]->id_Presidente;
        /**
        ...
        foreach ($listeSingoloPresidente as $singolaLista) {
            if ($comuneInCorso == $singolaLista['Istat Comune']) {
                ...
                if (!array_key_exists($this->numeroListaProvincia, $this->jsonObject->cand[$this->numeroCandidatoProvincia]->liste)) {
                    ...
                    $this->jsonObject->cand[$this->numeroCandidatoProvincia]->liste[$this->numeroListaProvincia]->img_lis_c = $singolaLista['img_lis_c']; 
 ...
 */                 
                    if ($singolaLista['Voti'] > 0 && $voti_validi_provincia> 0) {
                        $percVotiLista = round((($singolaLista['Voti']/$voti_validi_provincia)*100),2);
                    }
 ...
 */
                    if ($this->jsonObject->cand[$this->numeroCandidatoProvincia]->liste[$this->numeroListaProvincia]->voti > 0 && $voti_validi_provincia > 0) {
                        $percVotiLista = round((($this->jsonObject->cand[$this->numeroCandidatoProvincia]->liste[$this->numeroListaProvincia]->voti/$voti_validi_provincia)*100),2);
                    }
...
*/
    ...
    public function setCandidatoProvinciaBolzano($candidatoAr, $datiAffluenzaComuneInCorso) {
        ...
        $this->jsonObject->cand[$this->numeroCandidatoProvincia]->tot_voti_validi = $candidatoAr['voti_validi_prov'];; 
        ...
        $voti_validi_prov = $candidatoAr['voti_validi_prov'];
...
//        $votiValidi = $this->jsonObject->int->vot_t - ($candidatoAr['MUNI_BLANK'] + $candidatoAr['MUNI_NULLS']);
    //if ($candidatoAr['LIST_VOTES'] > 0 && $this->jsonObject->int->vot_t > 0) {
/*
     if ($candidatoAr['LIST_VOTES'] > 0 && $voti_validi_prov > 0) {
            $percVoti = round((($candidatoAr['LIST_VOTES']/$voti_validi_prov)*100),2);
        }
 */
 ...
 */     
        if ($this->jsonObject->cand[$this->numeroCandidatoProvincia]->voti > 0 && $voti_validi_prov > 0) {
            $percVoti = round((($this->jsonObject->cand[$this->numeroCandidatoProvincia]->voti/$voti_validi_prov)*100),2);
        }
    ...
    public function setCandidatoProvinciaTrento($candidatoAr) {
            ...
            if (!property_exists($this->jsonObject->cand[$this->numeroCandidatoProvincia], 'pos')) {
                ...
                $this->jsonObject->cand[$this->numeroCandidatoProvincia]->img_lis_r = $candidatoAr['img_lis_c']; 
